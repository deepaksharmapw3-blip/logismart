...
export class AnalyticsService {
   ...
   */
  static calculateDeliveryTrends(shipments: Shipment[]) {
   ...
   */
  static calculateDelayReasons(predictions: DelayPrediction[]) {
   ...
   */
  static calculateRoutePerformance(predictions: DelayPrediction[]) {
   ...
   */
  static identifyBottlenecks(shipments: Shipment[]) {
    ...
    const bottlenecks = Array.from(locationMap.entries())
      .map(([location, data]) => {
        ...
        return {
          location,
          delay: delayPercent,
          severity: (delayPercent >= 50 ? 'high' : delayPercent >= 25 ? 'medium' : 'low') as "low" | "medium" | "high",
        };
      })
    ...
    return bottlenecks.length > 0 ? bottlenecks : [
      { location: 'LA Warehouse', delay: 45, severity: 'high' as const },
      { location: 'Denver Hub', delay: 28, severity: 'medium' as const },
    ];
  }
   ...
   */
  static calculateStats(shipments: Shipment[], predictions: DelayPrediction[]) {
  ...
  static async generateAnalytics(): Promise<AnalyticsData> {
    ...
    return {
      deliveryTrends: this.calculateDeliveryTrends(shipments),
      delayReasons: this.calculateDelayReasons(predictions),
      routePerformance: this.calculateRoutePerformance(predictions),
      bottlenecks: this.identifyBottlenecks(shipments),
      stats: this.calculateStats(shipments, predictions),
    };
  }
}
